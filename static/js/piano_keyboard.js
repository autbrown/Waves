...
(function($){
	$.widget('l90r.piano', {
		...
		_createKeys: function() {
			...
			var whiteWidth = this.options.whiteWidth;
			var whiteHeight = this.options.whiteHeight;
			var pos = obj.offset();
			...
			for(var i=this.options.start; i<this.options.start+this.options.keys; i++) {
				...
				if(layout[key][0] == "w") {
					$('<div/>').css('position','absolute')
						.offset( { left:pos.left + 1 + ((whiteWidth+1)*whiteCounter) })
						.addClass('piano-ivory piano-key piano-' + i)
						.appendTo(obj)
						.data('piano-key', i);
					whiteCounter++;
				} else {
					...
					$('<div/>').width(blackWidth).height(blackHeight).css('position','absolute')
						.offset( { left:pos.left + 1 + ((whiteWidth+1)*whiteCounter) + Math.round((-1+xshift)*blackWidth) })
